    ...
    public static class PlayerManager
    {
        ...
        public static int CheckEquipedItems(PlayerEquipedItems items, List<ItemsModel> inventory, bool BattleRules = false)
        {
            ...
            if (BattleRules)
            {
                if (!w1)
                {
                    if (items.Primary == 105025 || //SSG-69 (IN MODE)
                        items.Primary == 106007)   //870MCS (IN MODE)
                    {
                        w1 = true;
                    }
                }
            ...
            lock (inventory)
            {
                foreach (ItemsModel item in inventory)
                {
                    if (item._count > 0)
                    {
                        if (item._id == items.Primary) w1 = true;
        ...
        {
            if (items.Primary != source.Primary) query.AddQuery("weapon_primary", source.Primary);
        ...
        {
            query.AddQuery("weapon_primary", items.Primary);
