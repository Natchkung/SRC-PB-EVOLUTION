                    ...
                    if (room != null)
                    {
                        if (str.StartsWith("NS") && player.access >= 0 && !room.GameRuleActive)
                        {
                            if (room.getLeader() == player)
                            {
                                ...
                                {
                                    SendMe = true;
                                    text = "คำสั่งนี้ใช้งานได้เฉพาะยังไม่เริ่มเกมส์เท่านั้น";
                                }
                            }
                            else
                            {
                        ...
                        else if (str.StartsWith("GR") && player.access >= 0 && (room.BarrettActive || room.ShotgunActive || room.MaskActive || room.RPG7Active) && !room.RuleFlag.HasFlag(GameRuleFlag.กฎแข่ง))
                        {
                            if (room.getLeader() == player)
                            {
                                SendMe = true;
                                ...
                                if (room.RoomState != RoomState.Battle)
                                {
                                    if (!room.SniperMode && !room.ShotgunMode)
                                    {
                                        ...
                                        if (room.RPG7Active)
                                        {
                                            ...
                                            room.RuleFlag -= 16;
                                            text = "ปิดใช้งานโหมดปืน RPG";
                                        }
                                        else
                                        {
                                            room.RPG7Active = true;
                                            room.RuleFlag += 16;
                                            text = "เปิดใช้งานโหมดปืน RPG";
                                        }
                                    }
                                    else
                                    {
                        ...
                        /*if (str.StartsWith("WC") && player.access >= 0 && (!room.BarrettActive && !room.ShotgunActive && !room.MaskActive && !room.RPG7Active))
                        {
                            if (room.getLeader() == player)
                            {
                                ...
                                if (room.GameRuleWCActive)
                                {
                                    room.GameRuleActive = false;
                                    room.RuleFlag -= 9;
                                    text = "ปิดใช้งานโหมดกฎแข่งเลือด 100";
                                    room.updateSlotsInfo();
                                }
                                else
                                {
                                    ...
                                    room.RuleFlag += 9;
                                    text = "เปิดใช้งานโหมดกฎแข่งเลือด 100";
                                    room.updateSlotsInfo();
                                }
                            }
                            else
                        ...
                        else if (str.StartsWith("WC") && player.access >= 0 && (room.BarrettActive || room.ShotgunActive || room.MaskActive || room.RPG7Active) && !room.RuleFlag.HasFlag(GameRuleFlag.กฎแข่ง))
                        {
                            if (room.getLeader() == player)
                            {
                                SendMe = true;
