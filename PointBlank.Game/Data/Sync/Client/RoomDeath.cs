...
namespace PointBlank.Game.Data.Sync.Client
{
    public static class RoomDeath
    {
        public static void Load(ReceiveGPacket p)
        {
            ...
            int num2 = p.getBuffer().Length;
            if (p.getBuffer().Length > (29 + Length))
            {
            ...
            if (room != null && room.round.Timer == null && room.RoomState == RoomState.Battle)
            {
                ...
                if (killer != null && killer.state == SlotState.BATTLE)
                {
                    ...
                    for (int i = 0; i < killsCount; i++)
                    {
                        ...
                        float vicY = p.readT();
                        float vicZ = p.readT();
                        byte vicflag = p.readC();
                        int Assist = p.readC();
                        int victimId = deathInfo & 15;
                        Slot victim = room.getSlot(victimId);
                        if (victim != null && victim.state == SlotState.BATTLE)
                        {
                            Frag frag = new Frag(deathInfo) { flag = vicflag, AssistSlot = Assist, victimWeaponClass = weaponClass, x = vicX, y = vicY, z = vicZ };
        ...
        public static void EndBattleByDeath(Room room, Slot killer, bool isBotMode, bool isSuicide)
        {
            ...
            else if (!killer.specGM && (room.RoomType == RoomType.Bomb || room.RoomType == RoomType.Annihilation || room.RoomType == RoomType.Convoy || room.RoomType == RoomType.Ace))
            {
                int winner = 0;
                int allRed;
                int allBlue;
                ...
                if (room.RoomType == RoomType.Ace)
                {
                    ...
                    if (slot0._deathState == DeadEnum.Dead)
                    {
                        winner = 1;
                        room.blue_rounds++;
